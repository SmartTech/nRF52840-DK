...
nrf_drv_uart_t uart_0 = NRF_DRV_UART_INSTANCE(0);
nrf_drv_uart_t uart_1 = NRF_DRV_UART_INSTANCE(1);
...
static uart_control_block_t m_cb[UART_INSTANCE_COUNT];

nrf_drv_uart_t* getInstance(uint8_t instance) {
  if(instance==1) return &uart_1;
  return &uart_0;
}
